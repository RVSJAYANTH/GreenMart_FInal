...
const Navbar = () => {
  ...
  return (
    ...
    <NavLink to="/" onClick={()=>setOpen(false)}>
        <img className="h-9 w-35" src={assets.logo} alt="logo" />
    </NavLink>
    ...
    <div className="hidden sm:flex items-center gap-8">
        ...
        <div onClick={()=>navigate("/cart")}className="relative cursor-pointer">
           <img src={assets.cartLogo} alt="cart" className="w-6 opacity-80"/>
  ...
  >
    ...
    <ul
      ...
      py-2.5 w-[120px] rounded-md text-sm z-40 hidden group-hover:block"
    >
      <li
        onClick={() => navigate("/my-orders")}
        className="p-1.5 pl-3 hover:bg-green-100 cursor-pointer"
      >
        My Orders
      </li>
      <li
    ...
    <div className="flex items-center gap-6 sm:hidden">
       <div onClick={()=>navigate("/cart")}className="relative cursor-pointer">
        ...
        <button onClick={() => open ? setOpen(false) : setOpen(true)} aria-label="Menu" className="sm:hidden">
         ...
         <img src={assets.menu} alt="menu" className="h-4"/>
       </button>
    </div>
    ...
    {open && (
        ...
        {!user?( <button onClick={()=>{
            ...
            Login
             ...
             <button onClick={logout} className="cursor-pointer px-8 py-2 bg-green-500 hover:bg-green-600 transition text-white rounded-full">
             Logout
         </button>
        )}
